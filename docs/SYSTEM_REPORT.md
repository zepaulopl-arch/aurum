...
- `scripts/train.ps1` now closes with `PROFILE SUMMARY`, `SYSTEM CHECKS`, revised `VERDICT`, and `KEY FILES`.
- `scripts/weekend.ps1` has the same closing structure and reports scenario/pytest status from logs when the run reaches those steps.
...
- `.\scripts\train.ps1`: exit `0`, runtime `runtime\daily_train_20260604_211518`.
- `.\scripts\train.ps1`: showed profile summary, system checks as `NOT_RUN`, revised blocker verdict, and key files; no loose `0` lines were visible.
...
- A fresh `.\scripts\weekend.ps1` attempt at `runtime\weekend_full_20260604_212433` exceeded the tool timeout and was manually stopped while running `train --autotune --details`. Its manifest is correctly marked `FAIL`.
...
- Re-run `.\scripts\weekend.ps1` in a long enough shell session, or add a bounded quick weekend mode for development validation.
