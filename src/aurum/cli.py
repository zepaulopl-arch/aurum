...
from aurum import presets as presets_mod
from aurum import terminal_ui as ui
from aurum.cli_context import resolve_market_context_args
from aurum.cli_parsers import (
    add_basket_parser,
    add_borrow_parser,
    add_context_parser,
    add_db_parser,
...
def _run_short_lab_command(args: argparse.Namespace) -> int:
    ...
    from aurum.cli_predict import (
        resolve_single_horizon_dataset_output,
        resolve_single_horizon_evaluation_output,
    )
    from aurum.prediction_lab import render_prediction_lab_summary, run_prediction_lab
...
def _run_daily_command(args: argparse.Namespace) -> int:
    ...
    from aurum.cli_daily import run_daily_command
...
def _run_weekly_command(args: argparse.Namespace) -> int:
...
def build_parser() -> argparse.ArgumentParser:
        ...
        parser = argparse.ArgumentParser(
            prog="aurum",
            description="Aurum command line interface",
        )
        parser.add_argument(
            "--color",
        ...
        def add_mtm_parser(name: str, *, alias_of: str = "") -> None:
            ...
            mtm_parser.add_argument("--list", default="IBOV")
            mtm_parser.add_argument("--signals-dir", default="storage/signals")
            mtm_parser.add_argument("--signal-date", default="")
            mtm_parser.add_argument("--review-date", default="")
            mtm_parser.add_argument(
                "--capital",
                type=float,
                default=10000.0,
        ...
        daily_parser.add_argument("--universe", default="")
        daily_parser.add_argument("--list", default="IBOV")
        ...
        daily_parser.add_argument("--no-update", action="store_true")
        ...
        weekly_parser = subparsers.add_parser(
            "weekly",
            help="Run functional weekly train/audit routine",
        )
        weekly_parser.set_defaults(command="weekly")
        weekly_parser.add_argument("--list", default="IBOV")
        weekly_parser.add_argument("--output", default="storage/reports/latest_weekly_report.txt")
        weekly_parser.add_argument("--matrix", default="storage/features/latest_feature_matrix.csv")
        weekly_parser.add_argument("--universe", default="data/universes/ibov_live.csv")
        weekly_parser.add_argument("--prices-dir", default="data/prices")
        ...
        weekly_parser.add_argument("--no-update", action="store_true")
        weekly_parser.add_argument("--no-train", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        from aurum.cli_context_engine import run_context_engine_argv
    ...
    if (
        ...
        from aurum.cli_context_audit import run_context_audit_argv
    ...
    try:
        ...
        if args.command == "weekly":
            return _run_weekly_command(args)
...
def _run_prices_command(args: argparse.Namespace) -> int:
    from aurum.cli_prices import run_prices_command

    return run_prices_command(args)


def _run_universe_command(args: argparse.Namespace) -> int:
    from aurum.cli_universe import run_universe_command
