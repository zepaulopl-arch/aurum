...
def _ensure_review_report_bridge(run_dir: Path, profile: str) -> Path:
    ...
    default_bridge_dir = run_dir == Path("storage/runs/latest")

    if expected.exists() and not default_bridge_dir:
        return expected
    ...
    if default_bridge_dir:
        newest = _newest_existing(shared_candidates)
        if newest is not None:
            if newest != expected:
                shutil.copy2(newest, expected)
            return expected

    candidates = [expected, *shared_candidates]
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="python -m aurum review run")
    ...
    if not run_dir.exists():
        ...
        print("  python -m aurum signal run --profile CON --top 10 --list IBOV")
        raise SystemExit(2)

    try:
        bridged = _ensure_review_report_bridge(run_dir, args.profile)
        print(f"REVIEW BRIDGE | report {bridged}", flush=True)
    ...
    if result == 0:
        try:
            if args.force:
                ...
                reason = (
                    "review executed with --force; mechanical test only, "
                    "not elapsed performance review"
                )
            else:
                status = "OK"
