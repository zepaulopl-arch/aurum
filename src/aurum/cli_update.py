...
from aurum.artifact_metadata import artifact_metadata
from aurum.data.prices_csv import check_prices_dir
from aurum.data.prices_yahoo import fetch_yahoo_prices_from_ticker_file
from aurum.data.universe_builder import build_universe_csv_from_prices
from aurum.data.universe_csv import validate_universe_csv
from aurum.features_v2 import load_features_config, write_features_v2
from aurum.indices_prices import check_indices_prices_dir, fetch_indices_prices
from aurum.market_context import validate_market_context
from aurum.market_context_auto import write_auto_market_context
from aurum.market_context_consolidator import write_market_context
from aurum.market_context_sources import render_source_diagnostics
    ...
    try:
        ...
        if status != "OK":
            return _fail_payload(
                list_name=list_text,
                step="features_check",
                reason="features config is invalid",
...
def render_update_summary(payload: dict[str, Any]) -> str:
    ...
    features_step = _find_step(payload.get("steps", []), "features") or {}
    features_payload = features_step.get("payload", {})
    if isinstance(features_payload, dict) and features_payload.get("feature_set"):
        lines.extend(
            [
                "",
                "FEATURES",
                "--------------------------------------------------------------------------------",
                f"feature_set          {features_payload.get('feature_set', '-')}",
                f"features_total       {features_payload.get('features_total', 0)}",
                f"features_used        {features_payload.get('features_used', 0)}",
                f"feature_audit        {features_payload.get('feature_audit', '-')}",
                f"matrix               {features_payload.get('matrix', features_payload.get('output', '-'))}",
            ]
        )
