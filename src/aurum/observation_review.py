...
from aurum.data.prices_csv import read_price_rows_csv
...
def _d1xd_points(
    *,
    ...
    """Return the previous and current trading closes for D-1 x D review.

    The primary review contract is price-file based: D is the latest local close
    available for the ticker, and D-1 is the previous trading row in that file.
    The report reference price is only a single-day fallback for smoke tests and
    very small fixtures.
    """
    ...
    previous = [p for p in ordered[:-1] if p.date < current.date]
    if previous:
    ...
    if not target_date or current.date >= target_date:
        reference = PricePoint(date=target_date or current.date, close=float(ref_price))
        return reference, current
